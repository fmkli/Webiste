...
function Init(feedType, feedPath, bgArr, senArr) {
    ...
    feed.addEventListener("load", function () {$(document).ready(function(){
            $.get("https://kkapi.imfmkli.top/api/ispeak?author=62fb537eea70bbc9782bcf6d&page=1&pageSize=5",function(data2){
                    if (data2.data.items[0].title == ""){
                        data2.data.items[0].title = "鸽子暂未设置标题"
                    }
                    document.getElementById("speakin").innerHTML="<br><h2>最新说说</h2>标题：<br>" + data2.data.items[0].title + "<br>内容：<br>" + data2.data.items[0].content + "<br>初次上传时间：<br>" + data2.data.items[0].createdAt + "<br>"
                    console.log("1")
            });
    });
    });
    feed.addEventListener("error", function () {
        ...
        setArtTip('<i class="fa fa-warning"></i><p class="article-empty-tip">暂时无法连接到说说</p>');
    });
    feed.send();
